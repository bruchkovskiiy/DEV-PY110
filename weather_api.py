...
def current_weather(lat: float, lon: float) -> dict:
    """
    Функция получает погоду для указанных координат.
    На входе lat - широта, lon - долгота.
    На выходе получаем словарь с данными о погоде.
    """
    token = '7deb07b7-081e-4612-9bb2-0f6632f0eba0'  # Вставить ваш токен из api.weatherapi.com
    ...
    result = {
        'city': data['info']['tzinfo']['abbr'],  # Город
        'time': datetime.fromtimestamp(data['fact']['uptime']).strftime("%H:%M"),  # Время обновления данных
...
if __name__ == "__main__":
    pprint(current_weather(59.8944, 30.2642))  # Проверка работы для координат Санкт-Петербурга
